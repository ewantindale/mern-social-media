...
router.get('/', auth, (req, res) => {
    User.findById(req.user.id).then(user => 
      Post.find({ authorId: [...user.following, req.user.id] }).sort({date:-1}).then(posts => {
        posts.forEach(post => {
  ...
  Post.findById(req.params.id)
    .then(post => {
      ...
      if (updatedPost.likedBy.includes(req.user.id)){
        updatedPost.likedBy = updatedPost.likedBy.filter(e => e !== req.user.id)
      } else {
...
router.delete('/:id', auth, (req, res) => {
    Post.findById(req.params.id)
        .then(post => {
          if (req.user.id !== post.authorId) return res.status(401).json({ msg: 'Incorrect user ID, authorization denied' });
